...
- **控制台深蓝** (#004782)：品牌标识、主要按钮、输入焦点和关键链接。
- **交互蓝** (#185fa5)：当前导航、主要按钮悬停和强调型数据。
- **信息冰蓝** (#ecf4ff)：侧栏、只读提示、选中区域和轻量信息容器。
- **交互浅蓝** (#d6e4f3)：悬停反馈，不作为大面积背景。

### Secondary

- **P1 警报红** (#C92A2A)：仅用于重大风险、危险操作和错误强调。
- **P2 警戒橙** (#D97706)：高风险和警告状态。
- **P3 分析蓝** (#2563EB)：中度风险，不与品牌主操作混用。
- **P4 稳定灰** (#64748B)：低风险和次要等级。
- **运行绿** (#10B981)：服务正常、监控启用和成功反馈。

### Neutral

- **冷白画布** (#f7f9ff)：浅色模式页面背景与输入底色。
...
**一屏一主操作规则。** #004782 用于当前屏幕最重要的操作；次要动作使用白底描边或透明背景。
...
系统以描边和色调分层为主、阴影为辅。静态卡片通常使用 1px #c2c6d2 描边与极轻阴影；粘性顶栏、下拉面板和移动导航使用更明确但仍柔和的阴影；弹窗才使用最高层级阴影。深色模式主要依靠 #101d28、#0f172a 与 #1e293b 的明度差建立层级。
...
- **Primary:** #004782 白字，典型内边距 10px 20px；文字 13–14px、600–700。
- **Hover / Focus:** 悬停切换为 #185fa5，并允许轻微 0.97–1.02 缩放；键盘焦点使用 2px #004782 焦点环。
- **Secondary / Ghost:** 白底或透明底、#c2c6d2 描边、#004782 或 #424751 文字；不得与主按钮争夺视觉优先级。
- **Disabled / Loading:** 降低透明度但保持标签可读；异步动作必须显示进行中状态并阻止重复提交。

### Chips

- **Style:** 4px 8px 内边距、全圆角、11–12px 粗体；信息类使用 #ecf4ff/#004782。
- **State:** P1–P4 芯片严格采用风险语义色；选中筛选器可使用主蓝描边或浅蓝背景。

### Cards / Containers

- **Corner Style:** 标准卡片 12px，关键面板和弹窗 16px。
- **Background:** 浅色使用 #ffffff，辅助区使用 #f7f9ff 或 #ecf4ff；深色使用 #101d28、#0f172a、#1e293b。
...
- **Style:** #f7f9ff 背景、1px #c2c6d2 描边、8–12px 圆角、10px 12px 内边距。
- **Focus:** 2px #004782 焦点环；不得只移除浏览器 outline 而没有替代状态。
- **Error / Disabled:** 错误使用 #ffdad6 容器与 #93000a 文字；禁用仍需保持至少 4.5:1 的标签对比度。

### Navigation

- 桌面端固定 240px 侧栏，当前项使用 #185fa5 实心背景、白字和共享布局高亮；非当前项为 #424751，悬停进入 #d6e4f3/60。
