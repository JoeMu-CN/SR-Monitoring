...
- React/Vite 当前风险提醒查询页和 P1 至 P4 本页分布。
- FastAPI 健康检查接口。
...
生产发布使用独立的 Nginx/TLS 覆盖配置，不直接暴露 app：先复制并填写
`deploy/.env.production.example` 为 `deploy/.env.production`，准备 `deploy/certs/fullchain.pem`
和 `deploy/certs/privkey.pem`，再执行：

```powershell
docker compose --env-file deploy/.env.production -f compose.yaml -f compose.prod.yaml up -d --build
```
...
- 即时信源使用数据源自己的高频 cron；中央气象台旧默认周期会由迁移 `0025` 从每 30 分钟调整为每 5 分钟。管理员修改数据源周期或启停后，Scheduler 最迟一分钟自动刷新注册表。
- 每天 08:00：配置 `RESEARCH_DAILY_TOPIC` 和 `RESEARCH_SCHEDULE_OWNER_USERNAME` 后，幂等创建一条日报研究任务。
- 每周一 08:30：配置 `RESEARCH_WEEKLY_TOPIC` 和同一归属管理员后，按自然周幂等创建一条周报研究任务。
...
- 已接入中央气象台与 OFAC 拉取式数据源；天眼查 MCP 作为统一数据源目录中的按需外部核查工具，端点、运行密钥、每日/每月额度和启停均在**数据源控制台**统一配置（密钥加密存库，仅显示末四位）；生产环境不读取天眼查 `.env` 配置，手工 JSON 导入作为合规补充。
...
数据源页面现已支持管理员配置数据源编码、官方入口、可信度、5 段 cron 调度周期、认证方式、登录配置、密钥引用和启停状态，并提供新增、编辑、删除及修改日志查询。所有业务接口均要求服务端会话；写接口必须通过 Origin + CSRF 校验，权限由当前会话用户的四角色矩阵决定。`X-User-Role` / `X-User-Id` 不参与授权，不能把浏览器端角色或管理员模式当作身份认证。
