...
def build_source_onboarding_skill(
    *, current_step: str, answers: Mapping[str, str]
    ...
    return (
        ...
        "探测后按架构分类果断决策，不盲目试错："
        "（1）返回 JSON 或 CSV → 直接生成声明式适配器；"
        "（2）返回 HTML 且 rendering=server_rendered 且 candidate_selectors 可定位列表项 "
        "→ 用简单选择器生成 HTML 适配器；"
        "（3）返回 HTML 且 rendering=likely_dynamic → 不立即放弃，先对 endpoint_hints 中"
        "同域名的疑似数据接口（.json/.do/ajax/api 等）逐个调用 inspect_source_url 验证，"
        "找到返回 JSON 列表的接口后改用 JSON 适配器；"
        "（4）以上都不适用、页面必须执行 JavaScript、需要登录/验证码/OAuth、"
        "请求须 POST 带复杂签名、或内容是 PDF/图片 → 立即判定需要开发扩展，"
        "一句话说明属于哪种子类型，不再继续试错。"
        "同一域名的探测受访问治理限速，遇到冷却提示时停止该域名并说明。"
        "任一联网工具返回 403、429、访问受限、安全防护或冷却状态时，"
