...
def build_source_onboarding_skill(
    *, current_step: str, answers: Mapping[str, str]
    ...
    return (
        ...
        "对探测返回 likely_dynamic 的页面，不要立即判定需要开发扩展："
        "先阅读 endpoint_hints 与 script_sources 中的线索，优先对同域名的疑似数据接口"
        "（.json、.do、ajax、api 等）逐个调用 inspect_source_url 验证；"
        "找到返回 JSON 列表的接口后，把采集 URL 换成该接口并按 JSON 适配器继续配置。"
        "同一域名的探测受访问治理限速，遇到冷却提示时停止该域名并说明。"
        "仅当所有线索都不可用、页面必须执行 JavaScript 或存在登录/验证码时，"
        "才判定需要开发扩展。"
