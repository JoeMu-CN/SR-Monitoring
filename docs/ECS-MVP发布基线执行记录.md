...
- 本机 `research-worker` 是负责人为实机测试主动启动的，运行 `topic_source_discovery` 模式。任务 `684` 在 Worker 停止前已经完成：状态为 `succeeded`，搜索请求计数 `1`，候选结果计数 `6`，成功保存来源 `2`，任务记录的成本和输入/输出 Token 均为 `0`。日志中 4 条“候选来源读取失败”对应其余候选未能读取，不代表任务整体失败。
- Worker 是在该任务已完成后才被停止，当前为 `Exited (0)`；未停止 App、PostgreSQL、RSSHub 或 Crawl4AI。该停止不构成对已完成任务的止损，也不撤销负责人对本机实机测试的授权；如需新的真实研究任务，应由负责人再次在前端点击“开始研究”，以保持每次外部调用可见且可追溯。

### 本机自动研究报告草稿实机验收（2026-08-15）
