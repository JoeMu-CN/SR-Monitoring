...
export const RiskAssistantView: React.FC<RiskAssistantViewProps> = ({
  ...
  const initialMessage: ChatMessage = {
    ...
    content: `您好！我是 **SR 风险查询助手**。我可以帮助您快速查询当前启用的重点供应商、生产地点、供应产品，以及筛选当前有效的 P1–P4 风险提醒。此外，我还支持对清单外企业发起一次性工商与司法核查，以及查询天眼查接口调用额度。
...
💡 **只读提示**：本助手为**只读查询助手**，不具备新增/修改业务数据、更改监控状态或自动触发处置的权限。`,
  ...
  const presetQueries = [
    ...
    '核查【杭州智造科技有限公司】的工商与司法风险（清单外）',
    '查询天眼查 API 今日和本月调用额度',
    '筛选供应【微电子元件】与【特种溶剂】的供应商',
  ];

  // Helper to process NLP query
  const handleSend = (textToSend?: string) => {
    ...
    setTimeout(() => {
      const responseMsg = generateAssistantResponse(queryText);
      setMessages((prev) => [...prev, responseMsg]);
      setIsTyping(false);
    }, 600);
  };

  // NLP simulated response generator
  const generateAssistantResponse = (query: string): ChatMessage => {
    ...
    if (lower.includes('额度') || lower.includes('调用') || lower.includes('天眼查额度')) {
      toolCalls.push({
        id: `tool-${Date.now()}-1`,
        toolName: 'tianyancha_get_budget_quota',
        description: '查询天眼查 API 接口调用配额与剩余额度',
        params: { product: 'Enterprise_Search_API' },
        durationMs: 42,
        status: 'success',
        resultCount: 1,
      });
      ...
      return {
        ...
        sender: 'assistant',
        timestamp,
        content: `已为您完成 **天眼查 API 配额** 实时查询。当前配额状态良好，未触发限流警戒线：`,
        toolCalls,
        data: {
          quotaCard: quota,
        },
    ...
    // 2. External Company Check (Non-list enterprise)
    if (
      lower.includes('核查') ||
      lower.includes('清单外') ||
      lower.includes('杭州智造') ||
      ...
      const targetCompany =
        query.match(/【(.*?)】/)?.[1] ||
        (query.includes('杭州') ? '杭州智造科技有限公司' : '华东特种高分子材料有限公司');

      toolCalls.push({
        id: `tool-${Date.now()}-1`,
        toolName: 'external_check_tianyancha',
        description: `调用天眼查开放接口对【${targetCompany}】进行一次性工商/司法检索`,
        params: { enterpriseName: targetCompany, scope: ['ic_info', 'judgements', 'abnormal_ops'] },
        durationMs: 310,
        status: 'success',
        resultCount: 1,
      });
      ...
      setQuota((prev) => ({
        ...prev,
        dailyUsed: Math.min(prev.dailyLimit, prev.dailyUsed + 1),
        monthlyUsed: Math.min(prev.monthlyLimit, prev.monthlyUsed + 1),
      }));

      const externalData: ExternalCompanyCheck = {
        companyName: targetCompany,
        registrationNo: '91330100MA2G' + Math.floor(100000 + Math.random() * 900000),
        legalPerson: '张建国',
        regCapital: '5,000 万人民币',
        ...
        keyBusiness: '工业自动化控制器研发、高分子材料加工与微电子配件销售',
      ...
      return {
        ...
        sender: 'assistant',
        timestamp,
        content: `已通过 **天眼查 API** 完成对清单外企业 **【${targetCompany}】** 的一次性外部核查。

⚠️ **注意**：该企业为清单外一次性核查，不会自动加入常态化监控清单，亦不会更新系统内预警得分。`,
        toolCalls,
        data: {
          externalCheckCard: externalData,
        },
      };
    }

    // 3. Risk Query (P1 - P4 or level filter)
    if (
      lower.includes('p1') ||
      lower.includes('p2') ||
      lower.includes('p3') ||
      ...
      if (lower.includes('p1') || lower.includes('严重') || lower.includes('极高')) {
        filtered = filtered.filter((r) => r.level === 'P1');
        levelLabel = 'P1 严重级';
      } else if (lower.includes('p2') || lower.includes('高风险')) {
        filtered = filtered.filter((r) => r.level === 'P2');
        levelLabel = 'P2 高风险级';
      } else if (lower.includes('p3') || lower.includes('中度') || lower.includes('警告')) {
        filtered = filtered.filter((r) => r.level === 'P3');
        levelLabel = 'P3 中度级';
      }

      if (lower.includes('深圳')) {
        filtered = filtered.filter((r) => r.location?.includes('深圳'));
      }

      toolCalls.push({
        id: `tool-${Date.now()}-1`,
        toolName: 'query_risk_alerts',
        description: `检索核心风险数据库 [筛选条件: Level=${levelLabel}]`,
        params: { status: 'valid', levelFilter: levelLabel },
        durationMs: 85,
        status: 'success',
      ...
      return {
        ...
        sender: 'assistant',
        timestamp,
        content: `为您找到 **${filtered.length}** 条符合条件（${levelLabel}）的有效风险提醒记录：`,
        toolCalls,
        data: {
          riskCards: filtered,
        },
    ...
    // 4. Supplier Query (By location, category, name, code)
    if (
      lower.includes('供应商') ||
      lower.includes('地点') ||
      lower.includes('产品') ||
      ...
      let filterDesc = '已启用重点供应商';

      if (lower.includes('深圳')) {
        filteredSups = filteredSups.filter((s) => s.productionLocation.includes('深圳'));
        filterDesc += ' [城市: 深圳]';
      }
      if (lower.includes('苏州') || lower.includes('江苏')) {
        filteredSups = filteredSups.filter((s) => s.productionLocation.includes('江苏') || s.productionLocation.includes('常州') || s.productionLocation.includes('苏州'));
        filterDesc += ' [地区: 江苏/苏州]';
      }
      if (lower.includes('微电子') || lower.includes('芯片')) {
        filteredSups = filteredSups.filter((s) => s.category.includes('微电子') || s.category.includes('芯片') || s.suppliedProduct.includes('芯片'));
        filterDesc += ' [品类: 微电子/芯片]';
      }
      if (lower.includes('溶剂') || lower.includes('化工')) {
        filteredSups = filteredSups.filter((s) => s.category.includes('溶剂') || s.category.includes('化工') || s.suppliedProduct.includes('溶剂'));
        filterDesc += ' [品类: 特种溶剂/化工]';
      }

      toolCalls.push({
        id: `tool-${Date.now()}-1`,
        toolName: 'query_suppliers_database',
        description: `检索重点供应商台账 [条件: ${filterDesc}]`,
        params: { filter: filterDesc },
        durationMs: 92,
        status: 'success',
        resultCount: filteredSups.length,
      });
      ...
      return {
        ...
        sender: 'assistant',
        timestamp,
        content: `查询完成，为您匹配到 **${filteredSups.length}** 家符合要求的重点供应商信息：`,
        toolCalls,
        data: {
          supplierCards: filteredSups,
        },
      };
    }
    ...
    return {
      ...
      sender: 'assistant',
      timestamp,
      content: `已为您执行智能关联查询。检索到相关重点供应商与风险提醒情况如下：`,
      toolCalls,
      data: {
        supplierCards: suppliers.slice(0, 2),
        riskCards: riskItems.slice(0, 1),
      },
    };
  };
          ...
          <div className="space-y-2 text-[12px]">
            <div>
              <div className="flex justify-between text-slate-500 text-[11px]">
                ...
                <span className="font-bold text-slate-800 dark:text-slate-200">
                  {quota.dailyUsed} / {quota.dailyLimit} 次
                </span>
              </div>
            ...
            <div>
              <div className="flex justify-between text-slate-500 text-[11px]">
                <span>本月使用</span>
                <span className="font-bold text-slate-800 dark:text-slate-200">
                  {quota.monthlyUsed} / {quota.monthlyLimit} 次
                </span>
              </div>
              <div className="w-full h-1.5 bg-slate-100 dark:bg-slate-800 rounded-full mt-1 overflow-hidden">
                <div
